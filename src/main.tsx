...
function OrdersTable({ state, orders, user, token, refresh, compact }: { state: AppState; orders: Order[]; user?: User; token?: string; refresh?: () => Promise<void>; compact?: boolean }) {
  ...
  return <div className="card"><h3>{compact ? 'Order Terbaru' : 'Daftar Order'}</h3><div className="table-wrap"><table><thead><tr><th>No Order</th><th>Mitra</th><th>Status</th><th>Total</th><th>Item</th><th>Aksi</th></tr></thead><tbody>{orders.map((order) => <tr key={order.id}><td><b>{order.orderNumber}</b><br /><small>{new Date(order.orderDate).toLocaleString('id-ID')}</small></td><td>{partnerName(state, order.partnerId)}<br /><small>{tierName(state, state.partners.find((p) => p.id === order.partnerId)?.tierId ?? '')}</small></td><td><span className={`status ${order.status}`}>{statusLabels[order.status]}</span></td><td><b>{formatIdr(order.grandTotal)}</b></td><td>{order.items.length} item</td><td><div className="actions"><button className="btn small" onClick={() => setSelected(order)}>Detail</button>{user && token && refresh && user.role !== 'partner' && validTransitions[order.status].map((target) => <button key={target} className="btn small" onClick={async () => { await api.updateOrderStatus(token, order.id, target, `Update ke ${target}`); await refresh(); }}>{statusLabels[target]}</button>)}</div></td></tr>)}</tbody></table></div>{selected && <OrderModal state={state} order={selected} onClose={() => setSelected(null)} />}</div>;
}
...
function Products({ state }: { state: AppState }) { return <div className="card"><h3>Produk Frozen Food</h3><div className="table-wrap"><table><thead><tr><th>SKU</th><th>Produk</th><th>Kategori</th><th>MOQ</th><th>Status</th></tr></thead><tbody>{state.products.map((p) => <tr key={p.id}><td><b>{p.sku}</b></td><td>{p.name}<br /><small>{p.description}</small></td><td>{state.categories.find((c) => c.id === p.categoryId)?.name}</td><td>{p.minimumOrderQty} {p.unit}</td><td><span className="status delivered">Aktif</span></td></tr>)}</tbody></table></div></div>; }
function Partners({ state }: { state: AppState }) { return <div className="card"><h3>Data Mitra</h3><div className="table-wrap"><table><thead><tr><th>Kode</th><th>Nama Bisnis</th><th>Tier</th><th>Kontak</th><th>Termin</th><th>Status</th></tr></thead><tbody>{state.partners.map((p) => <tr key={p.id}><td>{p.partnerCode}</td><td><b>{p.businessName}</b><br /><small>{p.address}</small></td><td>{tierName(state, p.tierId)}</td><td>{p.contactPerson}<br /><small>{p.phone}</small></td><td>{p.paymentTermDays} hari</td><td><span className={`status ${p.status === 'active' ? 'delivered' : 'cancelled'}`}>{p.status}</span></td></tr>)}</tbody></table></div></div>; }
function Pricing({ state }: { state: AppState }) { return <div className="card"><h3>Harga Produk per Tier</h3><div className="table-wrap"><table><thead><tr><th>Produk</th>{state.tiers.map((t) => <th key={t.id}>{t.name}</th>)}</tr></thead><tbody>{state.products.map((p) => <tr key={p.id}><td><b>{p.name}</b><br /><small>{p.sku}</small></td>{state.tiers.map((t) => <td key={t.id}>{formatIdr(state.prices.find((price) => price.productId === p.id && price.tierId === t.id)?.price ?? 0)}</td>)}</tr>)}</tbody></table></div></div>; }
...
function Documents({ state, user, token, refresh }: { state: AppState; user: User; token: string; refresh: () => Promise<void> }) {
  ...
  const partner = findPartnerForUser(state, user);
  const orders = user.role === 'partner' && partner ? state.orders.filter((o) => o.partnerId === partner.id) : state.orders;
  return <div className="grid"><div className="card"><h3>Generate & Print Dokumen</h3><div className="table-wrap"><table><thead><tr><th>Order</th><th>Mitra</th><th>Invoice</th><th>Surat Jalan</th><th>Aksi</th></tr></thead><tbody>{orders.map((order) => { const inv = state.invoices.find((i) => i.orderId === order.id && i.status !== 'void'); const dn = state.deliveryNotes.find((d) => d.orderId === order.id && d.status !== 'void'); return <tr key={order.id}><td><b>{order.orderNumber}</b><br /><span className={`status ${order.status}`}>{statusLabels[order.status]}</span></td><td>{partnerName(state, order.partnerId)}</td><td>{inv ? <button className="btn small" onClick={() => setDoc({ type: 'invoice', invoice: inv })}>{inv.invoiceNumber}</button> : '-'}</td><td>{dn ? <button className="btn small" onClick={() => setDoc({ type: 'dn', deliveryNote: dn })}>{dn.deliveryNoteNumber}</button> : '-'}</td><td><div className="actions">{user.role !== 'partner' && <><button className="btn small" onClick={async () => { await api.createInvoice(token, order.id); await refresh(); }}>Generate Invoice</button><button className="btn small" onClick={async () => { await api.createDeliveryNote(token, order.id, { driverName: 'Driver Demo', vehicleNumber: 'B 1234 XYZ' }); await refresh(); }}>Generate SJ</button>{inv && inv.amountDue > 0 && <button className="btn small success" onClick={async () => { await api.recordPayment(token, inv.id, { amount: Math.min(500000, inv.amountDue), method: 'bank_transfer', referenceNumber: 'PAY-DEMO' }); await refresh(); }}>Catat Bayar</button>}</>}</div></td></tr>; })}</tbody></table></div></div>{doc && <DocumentModal state={state} doc={doc} onClose={() => setDoc(null)} />}</div>;
}
...
function Leaderboard({ state }: { state: AppState }) { return <div className="card"><h3>Leaderboard Bulan Berjalan</h3><div className="table-wrap"><table><thead><tr><th>Rank</th><th>Mitra</th><th>Tier</th><th>Delivered GMV</th><th>Total Qty</th><th>Order</th><th>Poin</th></tr></thead><tbody>{getLeaderboard(state).map((row) => <tr key={row.partnerId}><td><b>#{row.rank}</b></td><td>{row.partnerName}</td><td>{row.tier}</td><td>{formatIdr(row.totalOrderValue)}</td><td>{row.totalOrderQty}</td><td>{row.totalOrders}</td><td><b>{row.points}</b></td></tr>)}</tbody></table></div><p className="footer-note">Hanya order delivered yang dihitung agar ranking tidak dimanipulasi dari pending/cancelled.</p></div>; }
