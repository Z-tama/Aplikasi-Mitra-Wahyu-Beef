...
function OrderModal({ state, order, user, token, refresh, onClose }: { state: AppState; order: Order; user?: User; token?: string; refresh?: () => Promise<void>; onClose: () => void }) { return <div className="modal-backdrop"><div className="modal order-modal"><button className="modal-x" type="button" aria-label="Tutup detail order" onClick={onClose}><X size={20} /></button><div className="topbar"><div><h2>{order.orderNumber}</h2><p>{partnerName(state, order.partnerId)} • {statusLabels[order.status]}</p></div><button className="btn desktop-modal-close" onClick={onClose}>Tutup</button></div><DocumentOrder state={state} order={order} />{user && token && refresh && user.role !== 'partner' && <OrderShippingPanel order={order} token={token} refresh={refresh} />}</div></div>; }
...
function DocumentModal({ state, doc, onClose }: { state: AppState; doc: { type: 'invoice'; invoice: Invoice } | { type: 'dn'; deliveryNote: DeliveryNote }; onClose: () => void }) {
  const order = state.orders.find((o) => o.id === (doc.type === 'invoice' ? doc.invoice.orderId : doc.deliveryNote.orderId))!;
  const documentRef = useRef<HTMLDivElement>(null);
  return <div className="modal-backdrop print-backdrop"><div className="modal print-modal"><div className="actions no-print" style={{ justifyContent: 'flex-end' }}><button className="btn" onClick={() => printDocumentOnly(documentRef.current)}>Print / PDF</button><button className="btn" onClick={onClose}>Tutup</button></div><div ref={documentRef}>{doc.type === 'invoice' ? <InvoiceDocument state={state} invoice={doc.invoice} order={order} /> : <DeliveryDocument state={state} deliveryNote={doc.deliveryNote} order={order} />}</div></div></div>;
}
